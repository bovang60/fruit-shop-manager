...
public interface OrderRepository extends JpaRepository<Order, Integer> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT new com.fruitshop.backend.dto.DashboardDto$MonthlyPerformanceDto(" +
            ...
            "COUNT(o), " +
            "SUM(CASE WHEN o.status = com.fruitshop.backend.model.Order.OrderStatus.CANCELLED THEN 1 ELSE 0 END), " +
            "SUM(CASE WHEN o.status = com.fruitshop.backend.model.Order.OrderStatus.COMPLETED THEN (o.subTotal + o.shippingFee) ELSE 0 END)) " +
            "FROM Order o GROUP BY YEAR(o.createdAt), MONTH(o.createdAt) " +
            "ORDER BY YEAR(o.createdAt) DESC, MONTH(o.createdAt) DESC")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT new com.fruitshop.backend.dto.DashboardDto$TopSellerDto(s.shopName, " +
            ...
            "WHERE o.status = com.fruitshop.backend.model.Order.OrderStatus.COMPLETED " +
            "GROUP BY s.shopId, s.shopName, s.status " +
            "ORDER BY SUM(oi.quantity) DESC")
    java.util.List<com.fruitshop.backend.dto.DashboardDto.TopSellerDto> findTopSellersByQuantity(org.springframework.data.domain.Pageable pageable);
}
