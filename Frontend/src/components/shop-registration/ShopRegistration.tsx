...
const ShopRegistration: React.FC = () => {
    ...
    const checkAuthAndStatus = async () => {
      ...
      if (canRegisterRes.data === false) {
        showNotice(
          canRegisterRes.message ||
            "Bạn không thể đăng ký thêm shop vào lúc này.",
          "Thông báo",
        );
        navigate("/home");
        return;
      ...
      if (statusRes.resultCd === 0 && statusRes.data) {
        ...
        if (shop.status === "REJECTED") {
          setRejectReason(
            shop.rejectReason || "Yêu cầu trước đó của bạn đã bị từ chối.",
          );
          ...
          setFormData((prev) => ({
            ...
            businessType:
              shop.shopType === "Cá nhân"
                ? "personal"
                : shop.shopType === "Công ty"
                  ? "company"
                  : "household",
          }));
        }
    ...
    const fetchMethods = async () => {
      ...
      try {
        ...
        if (res.resultCd === 0 && res.data) {
          ...
          if (showError) {
            showError(
              res.message || "Không thể tải phương thức vận chuyển",
              "Lỗi",
            );
          }
        }
  ...
  const checkTaxCode = async (taxCode: string) => {
    ...
    try {
      ...
      if (result.code === "00" && result.data) {
        ...
        const hasHouseholdKeyword =
          upperName.includes("HỘ KINH DOANH") ||
          upperName.includes("HO KINH DOANH");

        // Check for NNT ngừng hoạt động
        const statusCheckStr = `${apiName} ${apiStatusObj}`.toUpperCase();
        if (
          statusCheckStr.includes("NNT NGỪNG HĐ") ||
          statusCheckStr.includes("NNT NGUNG HD") ||
          statusCheckStr.includes("NNT NGỪNG HOẠT ĐỘNG") ||
          statusCheckStr.includes("NNT NGUNG HOAT DONG")
        ) {
        ...
        const isMatch =
          (isHouseholdType && hasHouseholdKeyword) ||
          (isCompanyType && !hasHouseholdKeyword);
    ...
    if (currentStep === 3) {
      if (!isValidTaxCode(formData.taxCode, formData.businessType)) {
        ...
        if (formData.businessType === "personal") {
          ...
          msg =
            "Mã số thuế của Hộ kinh doanh và Công ty phải gồm từ 10 đến 14 ký tự hợp lệ.";
        }
      ...
      if (taxStatus === "INVALID") {
        showWarning(
          "Mã số thuế này thuộc về Người nộp thuế đã ngừng hoạt động (NNT ngừng HĐ).",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.resultCd === 0) {
        ...
        if (
          apiMsg.includes("Tên cửa hàng đã tồn tại") ||
          apiMsg.includes("shopName")
        ) {
          ...
          apiMsg.includes("Mã số thuế đã được sử dụng") ||
          apiMsg.includes("taxCode")
        ) {
          setTaxStatus("taken");
  ...
  return (
    <>
      ...
      <LoadingModal
        isOpen={isSubmitting}
        message="Đang gửi đăng ký..."
        subMessage="Vui lòng không đóng trang"
        theme="green"
      />
    </>
  );
