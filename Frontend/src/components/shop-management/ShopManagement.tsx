...
const ShopManagement: React.FC = () => {
  const { showNotice, showConfirm, showError, showWarning, showPrompt } =
    usePopup();
  // UI State
  ...
  const loadShops = useCallback(async () => {
    ...
    try {
      ...
      if (response.resultCd === 0 && response.data) {
        const mappedShops: Shop[] = response.data.content.map(
          (dto: ShopDto) => ({
            id: dto.shopId,
            ...
            orderCount: dto.totalOrders,
            cancelRate: dto.cancellationRate,
  ...
  const handleActivate = (id: number) => {
    showConfirm(
      `Bạn có muốn kích hoạt lại cho cửa hàng này hoạt động trở lại không?`,
      async () => {
        setLoading(true);
        try {
          const response = await activateShop(id);
          if (response.resultCd === 0) {
            showNotice(`Shop đã được kích hoạt lại thành công!`);
            setViewMode("LIST");
            loadShops();
          } else {
            showError(response.message || "Không thể kích hoạt lại cửa hàng");
          }
        } catch (error) {
          showError("Lỗi kết nối khi kích hoạt lại cửa hàng");
        } finally {
          setLoading(false);
        }
      },
