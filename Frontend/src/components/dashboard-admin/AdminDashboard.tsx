    ...
    try {
      ...
      if (response.resultCd === 0 && response.data) {
        ...
        const maxOrders = Math.max(
          ...data.ordersLast7Days.map((d: any) => d.orderCount),
          1,
        );

        const transformedOrders = data.ordersLast7Days.map(
          (d: any, i: number) => {
        ...
        setWeeklyOrders(
          transformedOrders.length > 0
            ? transformedOrders
            : [{ label: "N/A", value: "0 đơn", x: 0, y: 110 }],
        );
      } else {
        showError(
          response.message || "Không thể tải dữ liệu thống kê từ hệ thống",
        );
      }
    } catch (error) {
