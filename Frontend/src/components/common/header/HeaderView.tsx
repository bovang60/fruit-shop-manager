      ...
      <div className="header-actions">
        ...
        {isSeller && (
          <div className="notif-container" ref={notifRef}>
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="notif-icon">
                <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
              </svg>
              {newOrderCount > 0 && (
                <span className="notif-badge">{newOrderCount > 99 ? '99+' : newOrderCount}</span>
              )}

              <span className="notif-count-outside" aria-label={`Co ${newOrderCount} thong bao moi`}>
                <svg
                ...
                <span className="notif-count-outside-text">{newOrderCount > 99 ? '99+' : newOrderCount}</span>
              </span>
            </button>
            {isNotifOpen && (
              <div className="notif-dropdown">
                ...
                {pendingOrders.length === 0 ? (
                  <div className="notif-empty">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="notif-empty-icon">
                      <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
                    </svg>
                    ...
                    {pendingOrders.slice(0, 10).map(order => (
                      ...
                      >
                        ...
                        <div className="notif-item-body">
                          <p className="notif-item-title">Đơn #{order.orderId} — {order.receiverName}</p>
                          <p className="notif-item-sub">
                            ₫{order.totalAmount.toLocaleString('vi-VN')}
                            <span className="notif-item-time">
                              {new Date(order.createdAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}
                            </span>
                            <span className="notif-item-dot" aria-hidden="true"></span>
                          </p>
                        </div>
                      </li>
                    ))}
